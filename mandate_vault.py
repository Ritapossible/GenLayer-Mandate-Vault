...
def _parse_address(raw: str, field: str) -> Address:
    ...
    """
    if not isinstance(raw, str):
        raise gl.vm.UserError(f"{field} must be a string")
    try:
        return Address(raw)
    except Exception:
        raise gl.vm.UserError(f"{field} is not a valid address") from None
...
class MandateVault(gl.Contract):
    ...
    def __init__(
        ...
        if not isinstance(clauses, list) or not clauses:
            raise gl.vm.UserError("mandate must have at least one clause")
        for text in clauses:
            if not isinstance(text, str) or not text.strip():
                raise gl.vm.UserError("clause text must be non-empty")
            if len(text) > MAX_CLAUSE_CHARS:
                raise gl.vm.UserError("clause too long")
    ...
    def get_spend(self, spend_id: int) -> dict:
        if spend_id < 0 or spend_id >= len(self.spends):
            raise gl.vm.UserError("no such spend")
    ...
    def request_spend(self, payee: str, amount: int, memo: str) -> dict:
        ...
        if sender != normalize_address(self.owner.as_hex):
            if not self.agents.get(sender):
                raise gl.vm.UserError("sender is not an authorized agent")

        if not isinstance(memo, str):
            raise gl.vm.UserError("memo must be a string")
        if len(memo) > MAX_MEMO_CHARS:
            raise gl.vm.UserError("memo too long")
        payee_addr = _parse_address(payee, "payee")
    ...
    def _clause_verdict(
        ...
        def validator_fn(leader_res: gl.vm.Result) -> bool:
            ...
            if theirs["decision"] not in DECISIONS:
                return False
            if theirs["clause_id"] is not None and theirs["clause_id"] not in allowed:
                return False
